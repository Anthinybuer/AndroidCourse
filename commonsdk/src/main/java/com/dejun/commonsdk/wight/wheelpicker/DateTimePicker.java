package com.dejun.commonsdk.wight.wheelpicker;
...
public class DateTimePicker extends WheelPicker {
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        Logger.d("change months and days while set selected");
        ...
        if (timeMode != NONE) {
            selectedHour = TimeUtil.fillZero(hour);
            selectedMinute = TimeUtil.fillZero(minute);
        }
    }
    ...
    public void setSelectedItem(int yearOrMonth, int monthOrDay, int hour, int minute) {
        ...
        if (dateMode == MONTH_DAY) {
            ...
            Logger.d("change months while set selected");
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) && years.size() == 0) {
            Logger.d("init years before make view");
            initYearData();
        }
        if (dateMode != NONE && months.size() == 0) {
            Logger.d("init months before make view");
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            Logger.d( "init days before make view");
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
        ...
        if (timeMode != NONE && hours.size() == 0) {
            Logger.d(  "init hours before make view");
            initHourData();
        }
        if (timeMode != NONE && minutes.size() == 0) {
            Logger.d(  "init minutes before make view");
            changeMinuteData(TimeUtil.trimZero(selectedHour));
        }
        ...
        if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
            ...
            yearView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    Logger.d(  "change months after year wheeled");
                    if (resetWhileWheel) {
        ...
        if (dateMode != NONE) {
            ...
            monthView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
                        Logger.d(  "change days after month wheeled");
                        if (resetWhileWheel) {
                            selectedDayIndex = 0;//重置日子索引
                        }
                        int selectedYear;
        ...
        if (timeMode != NONE) {
            ...
            hourView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    Logger.d( "change minutes after hour wheeled");
                    changeMinuteData(TimeUtil.trimZero(selectedHour));
    ...
    private void changeMonthData(int selectedYear) {
        ...
        if (!resetWhileWheel) {
            ...
            Logger.d( "preSelectMonth=" + preSelectMonth);
        }
        months.clear();
    ...
    private void changeDayData(int selectedYear, int selectedMonth) {
        ...
        if (!resetWhileWheel) {
            ...
            Logger.d( "maxDays=" + maxDays + ", preSelectDay=" + preSelectDay);
        }
        days.clear();
    ...
    private void initHourData() {
        ...
        for (int i = startHour; i <= endHour; i++) {
            String hour = TimeUtil.fillZero(i);
            if (!resetWhileWheel) {
                if (i == currentHour) {
                    selectedHour = hour;
                }
