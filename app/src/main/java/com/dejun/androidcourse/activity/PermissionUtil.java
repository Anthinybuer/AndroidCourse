...
public class PermissionUtil {
    ...
    public static final int REQUEST_RECORD_AUDIO_CAMERA = 5;
     ...
     * 基本权限管理
     */
    static String[] BASIC_PERMISSIONS = new String[]{
            Manifest.permission.CAMERA,
   ...
   static List<String> mPermissionList = new ArrayList<>();
    @SuppressLint("Range")
    public static boolean hasCameraAndVideoPermission(Activity activity){
//        int hasPermission = ContextCompat.checkSelfPermission(activity,
//                Manifest.permission.RECORD_AUDIO);
...
//        return true;
        ...
        for (int i = 0; i < BASIC_PERMISSIONS.length; i++) {
            if (ContextCompat.checkSelfPermission(activity, BASIC_PERMISSIONS[i]) != PackageManager.PERMISSION_GRANTED) {
                mPermissionList.add(BASIC_PERMISSIONS[i]);
            }
        }
        if (mPermissionList.isEmpty()) {//未授予的权限为空，表示都授予了
            ...
            ActivityCompat.requestPermissions(activity, permissions, REQUEST_RECORD_AUDIO_CAMERA);//拍摄视频
            return false;
        }
    }
